...
@Slf4j
@Service
public class ClimateDataConsumer {

    @Autowired
    private SensordataService sensordataService;

    @RabbitHandler
    @RabbitListener(queues={"climate_data_backend_queue"})
    public void receiveMessages(Message message , Channel channel) throws IOException {
        String msg = new String(message.getBody());
        log.info("后端接收到Sensordata类型的消息 ---> " + msg);
        boolean ack = true;
        ...
        SensordataEntity jsonObject = JSONObject.toJavaObject(json , SensordataEntity.class);
        sensordataService.save(jsonObject);
    }
}
